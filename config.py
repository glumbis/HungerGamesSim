...
PLAYER_MIN_SPEED = 0.18     # pixels moved per frame, slowest player
PLAYER_MAX_SPEED = 0.45     # pixels moved per frame, fastest player
...
NEED_SECONDS_TO_EMPTY = {
    "hunger": 165,
    "thirst": 130,
    "sleep": 135,          # sleep runs out fastest, so players must rest regularly
}
...
LOOT_CENTER_SPREAD = 55     # typical distance (pixels) of cornucopia loot from the horn's mouth
...
RUSH_DURATION = 15          # seconds aggressive players rush the center at the start (the bloodbath lasts at least this long)
BLOODBATH_END_QUIET_SECONDS = 0.5  # the bloodbath only ends once no fight has been going on for this long
CENTER_PULL = 0.04          # exploring/searching destinations are moved this share of the way toward the middle (not for edge dwellers)
...
FLEE_DISTANCE = 800        # how far from the center fleeing players aim for
...
EXPLORE_MIN_TRIP = 650      # exploring players pick destinations at least this far away...
...
BLOODBATH_RADIUS = 100      # rushers start fighting (or backing off) this close to the cornucopia
...
CAMERA_OPENING_ZOOM = 1.3   # zoom on the cornucopia during the start
...
OUTCOME_WEIGHTS = {         # relative chance of each kind of fight outcome
    "ELIMINATION": 0.80,    # loser is eliminated, unless it escapes (then drops items and is injured)
...
ALLY_SUPPORT_RANGE = 110     # allies this close help in a fight
...
# --- Book features ---------------------------------------------------------
# Day and night
DAY_SECONDS = 90            # one full day (daytime + night) lasts this long
...
NIGHT_SLEEP_THRESHOLD = 97  # at night, tributes who aren't hunting go to sleep below this and sleep until dawn
...
# Opening clutter: fewer symbols around the cornucopia during the bloodbath
OPENING_DEATH_MARK_SECONDS = 4  # crosses where tributes fell in the bloodbath fade this quickly (no hearts at all)

# Camera and cards
CAMERA_MUTT_RADIUS = 160    # the automatic camera shows a mutt this close to a tribute
CARD_WIDTH = 300            # width of a tribute or alliance card (top left)

# Alliances act as one unit
ALLIANCE_ASSIST_RADIUS = 250  # members this close to an ally in a fight (or to the leader's hunt) join in
FOLLOW_CATCHUP_MULTIPLIER = 1.3  # a member further than FOLLOW_LEASH from its leader speeds up this much

# Big alliances at the start
...
PROFICIENT_WEAPON_FACTOR = 1.6  # strength with the weapon a tribute is proficient in
...
STEALTH_VISIBILITY = 0.7    # others see stealthy tributes only this far (share of normal)
SPEED_PROFICIENCY_FACTOR = 1.2  # fast runners move this much faster and escape more often
TRACKING_FACTOR = 2.0       # trackers sense unseen tributes this much further and chase longer

# Chilling: sometimes a tribute just stays put for a while
CHILL_CHANCE = 0.5          # chance, on reaching an exploring destination, to stay a while
CHILL_SECONDS = (6, 18)     # how long it stays

# Camera cooldown
CAMERA_SWITCH_COOLDOWN_SECONDS = 3  # the automatic camera stays on something at least this long before switching
CHILL_START_PER_SECOND = 0.05  # an idle tribute also sits down this often (chance per second) without arriving anywhere
