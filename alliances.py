...
used_names = set()  # alliance names already given out this game (cleared in main.Simulation)
records = {}        # alliance name -> most members it ever had (for the debrief awards)


def choose_name(first, second):
    """A name for a new alliance, e.g. "the Wolves". Two tributes from
    Districts 1, 2 or 4 found "the Careers" if that name is still free."""
    careers = {1, 2, 4}
    if first.district in careers and second.district in careers and "the Careers" not in used_names:
        name = "the Careers"
    else:
        free = [name for name in ALLIANCE_NAMES if name not in used_names and name != "the Careers"]
        name = random.choice(free) if free else f"Alliance {len(used_names) + 1}"
    used_names.add(name)
    return name
...
class Alliance:
    ...
    def add(self, player):
        ...
        records[self.name] = max(records.get(self.name, 0), len(self.members))
